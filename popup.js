...
function renderMain() {
    ...
    if (otherClasses.length > 0) {
        ...
        otherTitle.innerText = (favClasses.length > 0) ? 'Other Classes' : 'All Classes';
...
function renderGroup(container, dataObj, sortedKeys) {
  sortedKeys.forEach(className => {
    ...
    // Track the transition from favorited to non-favorited
    ...
    modules.forEach((modName, index) => {
      ...
      // If the previous module was favorited but this one isn't, add the separator
